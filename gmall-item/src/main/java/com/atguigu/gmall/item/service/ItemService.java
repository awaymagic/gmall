...
public class ItemService {
    ...
    public ItemVo loadData(Long skuId) {
        ...
        CompletableFuture<Void> catesFuture = skuFuture.thenAcceptAsync(skuEntity -> {
            ResponseVo<List<CategoryEntity>> catesResponseVo = this.pmsClient.queryLvl123WithSubsByCid3(skuEntity.getCategoryId());
        ...
        CompletableFuture<Void> saleAttrsFuture = skuFuture.thenAcceptAsync(skuEntity -> {
            ResponseVo<List<SaleAttrValueVo>> saleAttrsResponseVo = this.pmsClient.querySalesAttrValuesBySpuId(skuEntity.getSpuId());
        ...
        CompletableFuture.allOf(catesFuture,brandFuture,spuFuture,salesFuture,wareFuture,imagesFuture,saleAttrsFuture,saleAttrFuture,mappingFuture,descFuture,groupFuture).join();
        return itemVo;
    }
    ...
    public void generateHtml(ItemVo itemVo) {
        // 模版引擎的上下文对象，通过该对象对模版传递数据
        Context context = new Context();
        context.setVariable("itemVo", itemVo);

        try (PrintWriter printWriter = new PrintWriter("/Users/guowei/IdeaProjects/gmall/html/" + itemVo.getSkuId() + ".html")) {
            // 模版引擎生成静态页面，1-模版名称，2-上下文对象，3-文件流
